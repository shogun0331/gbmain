			...
			using( ZipStorer zip = ZipStorer.Create( ANDROID_RESOURCES_LIBRARY_PATH, "" ) )
			{
				...
				for( int i = 0; i < LocalizedData.Count; i++ )
				{
					...
					if( LocalizeAppNameOnAndroid )
					{
						...
						string aName = LocalizedData[i].AppName;
						 aName = aName.Replace("<","\\&lt;").Replace(">","\\&gt;").Replace("&","\\&amp;").Replace("\"","\\&quot;");
						
						string localizedStringsContents =
							"<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
							"<resources>\n" +
							"    <string name=\"" + AndroidNameResource + "\">" + aName + "</string>\n" +
							"</resources>";
	...
	public static class LocalizedAppTitlePostProcessBuild
	{
		...
		public static void OnPostprocessBuild( BuildTarget target, string buildPath )
		{
			...
			if( target == BuildTarget.iOS )
			{
				...
				// framework 추가 
				pbxProject.AddFrameworkToProject(unityFramework, "AppTrackingTransparency.framework", true);
				...
				foreach( LocalizedData localizedData in Settings.Instance.LocalizedData )
				{
					...
					if( Settings.Instance.LocalizeAppNameOniOS )
					{
						...
						if( File.Exists( localizedPlistPath ) )
						{
							...
							if(localizedData.LanguageCode == "en")
								localizedPlistContents = InsertValueToPlist( localizedPlistContents, "NSUserTrackingUsageDescription", "This identifier will be used to deliver personalized ads to you" );
							if(localizedData.LanguageCode == "ja")
								localizedPlistContents = InsertValueToPlist( localizedPlistContents,"NSUserTrackingUsageDescription", "この識別子は、個別に合わせた広告を提供するために使用されます。");
							if(localizedData.LanguageCode == "ko")
								localizedPlistContents = InsertValueToPlist( localizedPlistContents,"NSUserTrackingUsageDescription", "이 식별자는 개인 맞춤 광고를 제공하는데 사용됩니다.");
